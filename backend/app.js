...
// Подключение ejs
import { homepage } from "./api/page_ejs/homePage.js";
import { viewPage } from "./api/page_ejs/viewPage.js";
import { deletePage } from "./api/page_ejs/deletePage.js";
import { updatePage } from "./api/page_ejs/updatePage.js";
import { editBook } from "./api/page_ejs/editBook.js";
import { newBookPage } from "./api/page_ejs/newBookPage.js";
...
const app = express();
app.use(cors());
app.use(bodyParser.urlencoded({ extended: true })); // Нужно для того что бы парсить формы
...
app.get("/", homepage);
app.get("/books/view/:id", viewPage);
app.post('/books/delete/:id', deletePage)
app.get('/books/update/:id', updatePage)
app.post('/books/update/:id', editBook)
app.get('/books/newbook', newBookPage)
app.post("/books/newbook", uploadFile, newBook);


router.post("/user/login", async (req, res) => {     // авторизация пользователя
